...
class TemplateTypeHelper
{
	...
	public static function resolveTemplateTypes(
		...
		TemplateTypeVarianceMap $variances,
		TemplateTypeVariance $positionVariance,
		bool $keepErrorTypes = false,
	): Type
	{
		return VarianceAwareTypeTraverser::map($type, $positionVariance, static function (Type $type, TemplateTypeVariance $variance, callable $traverse) use ($standins, $variances, $keepErrorTypes): Type {
			if ($type instanceof TemplateType && !$type->isArgument()) {
				...
				$callSiteVariance = $variances->getVariance($type->getName());
