	...
	public function processNode(Node $node, Scope $scope): array
	{
		...
		foreach ($resolvedPhpDoc->getVarTags() as $name => $varTag) {
			...
			$errors = array_merge($errors, $this->genericObjectTypeCheck->check(
				...
				sprintf('Type projection %%s in generic type %%s in %s is conflicting with variance of template type %%s of %%s %%s.', $escapedIdentifier),
				sprintf('Type projection %%s in generic type %%s in %s is redundant, template type %%s of %%s %%s has the same variance.', $escapedIdentifier),
			));
